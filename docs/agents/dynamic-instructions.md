...
| `prompt_slug` | `str \| None` | No | References a control-plane registry prompt by slug. When set, `to_dict()` emits the slug and sends `system_prompt=""` so a pushed agent links to the governed prompt (see [Pushing agent definitions](../platform/index.md#registering-agent-definitions)). Does not affect local runtime, where `system_prompt` still drives the prompt. |
