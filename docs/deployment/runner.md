...
1. **Register** with your API key (`X-API-Key`) and advertise
   `capabilities:["live_playground","eval_run"]` (plus `tool_exec` when you pass
   `--tools` — see [Connectors](#connectors-tool_exec)). The platform returns a
...
tool's [`replay_class`](../tools/index.md#replay-safety-replay_class) and never
...
```text
execute: completed | result: {'success': True, 'result': 'OK'} | trace_id: cce3e8a4…
command status: completed   (recorded on the plane)
spans: [tool.ask_llm, agent.connector, llm.openai.gpt-4o-mini]
```
