...
method reads local SQLite, the test move touches no library code, and the rest
is files entering version control.

### Added

- **`TraceStore.list_spans()`** — reads spans across traces, oldest first, from
  a cursor. Every "show spans as they arrive" use case previously had to walk
  trace by trace: two streaming demos assumed a method like this one and were
  dead because of it, and example 08 dropped to raw `sqlite3`.

  `since` is a **cursor, not a timestamp**, and that is the whole point. A long
  span *starts* before a short one that finishes first, so it is written second
  while carrying the earlier start time. A timestamp cursor steps over it; a
  write-order cursor cannot. Optional `trace_id` and `execution_id` filters
  narrow the stream, the latter matching `chain.execution_id` so a durable run
  is followed across every trace it spans.

  The cursor rides on a new `SpanRecord` pair rather than on `SpanData`.
  `trace.platform_export.to_wire` dumps that model whole, so a field added to it
  would put a new key on the span payload the plane receives — a wire event
  under the cross-repo contract, for something the plane has no use for.

### Changed

- **The live-provider tests moved to `tests/e2e/` and are marked `e2e`.**
  `test_providers_live.py` calls Groq and Gemini for real, gated only on a key
  being present, so it made live calls from the deterministic suite on any
  developer machine that had them and failed transiently for provider reasons —
  a Gemini tool-call test failed once during the 1.70.0 work and passed four
  times either side of it. CI keeps provider keys in the e2e job, so it ran
  nowhere useful from `tests/integration/` anyway; now it runs in the gate that
  has them. Same remedy as the file moved in PR #160.

### Fixed

- **Six capture scripts existed only on one laptop.** `scripts/` is gitignored
  with twenty files force-added over time, and these six were never among them.
  Three of them regenerate the sprint-three screenshots, and all three of those
  images are published in the README and the docs — so images the project ships
  could not be regenerated by anyone else. Found immediately after fixing a
  screenshot that could not be regenerated for a different reason (1.71.0). Now
  tracked.
...
else — the `kb` extra, `all`, and the repo's own dev tree — and adds a way to do
local PDF work with no engine at all.

Nothing that works today stops working.

### Changed

- **`pymupdf` → `pypdfium2` everywhere.** The new engine is BSD-3-Clause +
  Apache-2.0 bindings to PDFium, Google's PDF engine (the one in Chrome).
  `fastaiagent[kb]` and `[all]` are now AGPL-free, as is the repo itself.

  Parity was verified before the swap, not assumed: across a corpus of
  multi-column text, ruled tables, rotated pages (0/90/180/270°), CJK + Cyrillic
  + Greek, an embedded raster, and a 12-page document — page counts and raster
  dimensions match at both 72 and 150 DPI, every text token pymupdf found is
  also found by pypdfium2, and the worst mean per-pixel difference is 1.21/255
  (anti-aliasing at glyph edges).

### Added

- **`pdf` extra** — `pip install "fastaiagent[pdf]"` for local PDF decoding
  without the embedding stack that `kb` pulls in. `kb` now depends on it.
- **Bring your own parser.** `PDF.from_file`, `from_bytes` and `from_url` take a
  keyword-only `text=`:
  ...
  `extract_text()` then returns it verbatim and **no PDF engine is involved**.
  Use whatever you already have — pdfplumber, pypdf, Tika, a vendor OCR API.
  The text survives `to_dict`/`from_dict`, so a checkpointed chain resumes with
  it intact. For vision without an engine, render pages yourself and pass the
  `Image` parts directly.
- **`MissingPDFBackendError`**, raised when local decoding is needed but unavailable.
  It names all three ways forward (the extra, `text=`, `pdf_mode="native"`).
  It subclasses **both** `MultimodalError` and `ImportError` deliberately: this
  path previously failed with a bare `ModuleNotFoundError`, so existing
  `except ImportError:` handlers keep working unchanged.

### Compatibility

`pdf_mode="native"` — what `pdf_mode="auto"` already selects for Claude
3.5/3.7/4.x, GPT-4o/4.1/5/o-series, Azure, Bedrock Claude and Gemini — forwards
raw bytes and has never needed a PDF engine. That path is untouched.

On a **core-only** install, `extract_text()` / `page_count()` /
`to_page_images()` and `pdf_mode="text"/"vision"` still need an engine. That was
already true in 1.55.0, where they raised a bare
`ModuleNotFoundError: No module named 'pymupdf'`; they now raise an actionable
error instead, and `text=` makes extraction work with no engine at all. Strictly
an improvement on that path — no API was removed.

The test fixture `contract.pdf` was regenerated by a dependency-free raw-PDF
writer (pypdfium2 reads PDFs but cannot author them), byte-different but
equivalent: same 2 pages, same 595×842 geometry, same text.
