...
def _summarize_trace(spans: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for sp in spans:
        ...
        if agent_name is None:
            agent_name = attrs.get("fastai.agent.name")
        if thread_id is None:
            thread_id = attrs.get("fastai.thread.id")
        total_cost += float(attrs.get("fastai.cost.total_usd") or 0.0)
...
def list_threads(
    ...
    try:
        ...
        for row in rows:
            ...
            tid = attrs.get("fastai.thread.id")
            if not tid:
                continue
